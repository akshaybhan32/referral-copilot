...
**Type a care need and a place — _"dialysis near Patna"_ — and get an evidence-attached shortlist of candidate health facilities, ranked by semantic relevance and distance.**

A Databricks hackathon app for social good: helping clinicians and patients in India find the right facility for a specific need, fast. Built on **Databricks Apps** (AppKit / TypeScript), **Lakebase** (Postgres + pgvector), and **Databricks Model Serving** (foundation-model embeddings), over ~10k real facilities from the Unity Catalog lakehouse.
...
┌─────────┐   /ask     ┌───────────────────────────────────────┐
│ NL box  │──────────► │ parse "<need> near <place>"            │
│         │            │ geocode place  ───────────────────────┼──► Lakebase (Postgres)
│ results │ ◄───────── │ embed need ───────────────────────────┼──► Model Serving
│ cards   │            │ search_facilities_vec(geo + cosine)  ──┼──► Lakebase pgvector
└─────────┘            │ attach query-relevant evidence (trgm)  │
                       └───────────────────────────────────────┘
                                                  ▲
...
- **Single serving endpoint.** The app uses the default `serving()` config (`DATABRICKS_SERVING_ENDPOINT_NAME`), injected from the `embed` serving-endpoint bundle resource.
